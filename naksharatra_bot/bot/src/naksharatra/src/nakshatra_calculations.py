...
def calc_nakshatra_tithi(
        location, time, time_format="%Y-%m-%d %H:%M",
        fs: float=8.0,
        filename="nakshatra_at_test_time.png"
        ):
    """
    ...
    naksh_names_tab = Table.read('nakshatram_names.tex',
                                 format="latex")
    ...
    nakshatram_coords = np.linspace(start_coord,
                                    start_coord + nakshatram_extent * 26,
                                    27)
    ...
    for coord_id, coord in enumerate(nakshatram_coords):
        ax.plot(np.deg2rad([coord, coord]), [1, 2.2], "#fff",
                alpha=0.3, )
        if coord > 360:
            nakshatram_coords[coord_id] = coord - 360
        text_theta = (2 * nakshatram_coords[
            coord_id] + nakshatram_extent) * (np.pi / 360)
        if 90 < coord < 270:
    ...
    ax.set_xticklabels([])
    # ax.set_xticklabels(nakshatram_coord_labels, fontsize=fs-2, )
    # ax.tick_params(axis='x', labelsize=fs - 2, pad=8)
    ...
    rashi_extent = 360 / 12
    rashi_coords = np.linspace(rashi_start,
                               rashi_start + rashi_extent * 11, 12)
    ...
    for coord_id, coord in enumerate(rashi_coords):
        ...
        rashi_text_theta = (2 * rashi_coords[
            coord_id] + rashi_extent) * (np.pi / 360)
        if (77 < coord < 255):
    ...
    plot_inner_graha_phase("mercuri", mercury_angle_to_sun,
                           [np.deg2rad(mercury_lambda), 1.9], 0.05,
                           fig, ax)
    plot_inner_graha_phase("venus", venus_angle_to_sun,
                           [np.deg2rad(venus_lambda), 1.9], 0.05, fig,
                           ax)
    ...
    known_eclipse_time = Time("2023-04-20 04:16:49", format="iso",
                              scale="utc")  ### This eclipse happens when moon was in Rahu's postion

    moon_lambda_known_eclipse = get_body("moon",
                                         known_eclipse_time).geocentrictrueecliptic.lon.value
    rahu_lambda, ketu_lambda = calc_rahu_ketu_pos(known_eclipse_time,
                                                  test_date_utc_time,
...
calc_nakshatra_tithi(location, date_str, fs=11.5,
                     filename="../../../nakshatra_a_test_time.png")
